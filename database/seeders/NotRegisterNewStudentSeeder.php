...
class NotRegisterNewStudentSeeder extends Seeder
{
    public function run(): void
    {
        $residentRole = Role::firstOrCreate(['name' => 'resident']);

        $filePath = database_path('data/new_students_not_reserved.csv');
    ...
    private function impData(array $data)
{
    ...
    private function insertArchiveData(array $data): int
    {
        ...
        $dataToInsert = [
            ...
            'academic_id' =>isset($data[4]) && strtoupper($data[4]) !== "NULL" ? $data[4] : null,
            'national_id' => isset($data[5]) && strtoupper($data[5]) !== "NULL" ? $data[5] : null,
            'faculty' => isset($data[25]) && strtoupper($data[25]) !== "NULL" ? $data[25] : null,
            'program' => null,
            'score' => isset($data[26]) && strtoupper($data[26]) !== "NULL" ? $data[26] : null,
            'percent' => isset($data[27]) && is_numeric($data[27]) && strtoupper($data[27]) !== "NULL" ? $data[27] : null,
            'academic_email' => $data[3] ,
            ...
            'governorate' => isset($data[10]) && strtoupper($data[10]) !== "NULL" ? $data[10] : null,
            'city' => isset($data[11]) && strtoupper($data[11]) !== "NULL" ? $data[11] : null,
            'street' => isset($data[12]) && strtoupper($data[12]) !== "NULL" ? $data[12] : null,
            'parent_name' => isset($data[13]) && strtoupper($data[13]) !== "NULL" ? $data[13] : null,
            'parent_mobile' => isset($data[14]) && strtoupper($data[14]) !== "NULL" ? $data[14] : null,
            'parent_email' => isset($data[15]) && strtoupper($data[15]) !== "NULL" ? $data[15] : null,
            'parent_is_abroad' => isset($data[16]) && $data[16] === 'مصر' ? 0 : 1,
            'parent_abroad_country' => isset($data[16]) && $data[16] !== 'مصر' ? $data[16] : null,


            'cert_type' => isset($data[17]) && strtoupper($data[17]) !== "NULL" ? $data[17] : null,
            'cert_country' => isset($data[18]) && strtoupper($data[18]) !== "NULL" ? $data[18] : null,
            'cert_year' => isset($data[19]) && is_numeric($data[19]) && strtoupper($data[19]) !== "NULL" ? (int) $data[58] : null,
            'sibling_name' => isset($data[21]) && strtoupper($data[21]) !== "NULL" ? $data[1] : null,
            'sibling_faculty' => isset($data[23]) && strtoupper($data[23]) !== "NULL" ? $data[23] : null,
            'has_sibling' => (isset($data[21]) && strtoupper($data[21]) !== "NULL") ? 1 : 0,
            'created_at' => now(),
            'updated_at' => now(),
        ];
    ...
    private function createUser(array $data, $residentRole): \App\Models\User
    {
        ...
        $academicEmail = isset($data[3]) && strtoupper($data[3]) !== "NULL" ? $data[3] : null;
